...
if ($action === 'pembeli_tambah_murid') {
    ...
    if ($nama === '' || $nisn === '' || $password === '') {
        $feedback = ['type' => 'error', 'msg' => 'Nama, NISN, dan password wajib diisi.'];
    } else {
        ...
        if ($cek) {
            ...
            $kelas_val = $id_kelas ? $id_kelas : 'NULL';
            if (mysqli_query($conn, "INSERT INTO murid (nama, nisn, password, id_kelas, status) VALUES ('$n','$ni','$h',$kelas_val,'aktif')")) {
                $feedback = ['type' => 'success', 'msg' => "Murid <strong>" . htmlspecialchars($nama) . "</strong> berhasil ditambahkan."];
            } else {
                $feedback = ['type' => 'error', 'msg' => 'Gagal menambahkan murid: ' . mysqli_error($conn)];
            }
        }
    }
}
