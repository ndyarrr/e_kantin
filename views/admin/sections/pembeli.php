...
<!-- ══ SEARCH & FILTER ══ -->
<form method="GET" action="" id="formFilterPembeli" style="margin-bottom:16px;display:flex;gap:10px;flex-wrap:wrap;align-items:center">
    ...
    <div style="position:relative;flex:1;min-width:200px">
        ...
        <input
            type="text"
            name="q_pembeli"
            value="<?= htmlspecialchars($searchPembeli) ?>"
            ...
            <table>
                ...
                <tbody>
                    <?php if (empty($daftarPembeli)): ?>
                        <tr class="empty-row">
                            <td colspan="5">
                                <i class="fa-solid fa-users" style="color:var(--green-muted);font-size:22px;display:block;margin-bottom:8px"></i>
                    ...
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge" style="background:<?= $isMurid ? 'var(--green-muted)' : '#e8f0fe' ?>;color:<?= $isMurid ? 'var(--green-dark,#2d7a2d)' : '#1a56db' ?>;font-weight:600;font-size:11px;padding:3px 10px;border-radius:20px">
                                    <?= $pb['kategori'] ?>
                                </span>
                            </td>
                            <td>
                            ...
                            <td class="center" style="white-space:nowrap">
                                ...
                                <button
                                    type="button"
                                    class="btn-aksi reset"
                                    title="Reset Password"
                                    onclick="bukaResetPembeli('<?= $idKey ?>','<?= $idVal ?>','<?= $resetAct ?>','<?= htmlspecialchars($pb['nama']) ?>')"
                                ><i class="fa-solid fa-key"></i></button>

                                <!-- Toggle Status -->
                                <form method="POST" style="display:inline" onsubmit="return confirm('<?= $aktif ? 'Nonaktifkan' : 'Aktifkan' ?> pembeli ini?')">
                                    ...
                                    <button type="submit" class="btn-aksi <?= $aktif ? 'toggle-off' : 'toggle-on' ?>" title="<?= $aktif ? 'Nonaktifkan' : 'Aktifkan' ?>">
    ...
    <div class="form-card">
        <div style="display:flex;gap:8px;margin-bottom:18px">
            <button
                ...
                onclick="switchTabPembeli('murid')"
            ...
            ><i class="fa-solid fa-graduation-cap" style="margin-right:5px"></i>Murid</button>
            <button
                id="tabGuru"
                type="button"
                class="btn-tab"
                onclick="switchTabPembeli('guru')"
            ...
            ><i class="fa-solid fa-chalkboard-teacher" style="margin-right:5px"></i>Guru</button>
        </div>
        ...
        <div id="formPanelMurid">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label>NISN</label>
                    <input type="text" name="nisn" placeholder="cth. 0012345678" required autocomplete="off" maxlength="20">
                </div>
                <div class="form-group">
                    <label>Password</label>
                    <div class="password-wrap">
                        <input type="password" name="password" id="inputPassMurid" placeholder="Buat password" required autocomplete="new-password">
                ...
                <div class="form-group">
                    <label>Kelas <span style="color:var(--text-light);font-weight:400">(opsional)</span></label>
                    <select name="id_kelas" class="form-select">
                        <option value="">Pilih kelas...</option>
                        <?php foreach ($semuaKelas as $kl): ?>
                            <option value="<?= $kl['id_kelas'] ?>">
                                <?= htmlspecialchars($kl['nama_kelas']) ?>
                                <?= $kl['nama_jurusan'] ? ' — ' . htmlspecialchars($kl['nama_jurusan']) : '' ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
        ...
        <div id="formPanelGuru" style="display:none">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label>NUPTK</label>
                    <input type="text" name="nuptk" placeholder="cth. 0012345678" required autocomplete="off" maxlength="20">
                </div>
                <div class="form-group">
                    <label>Password</label>
                    <div class="password-wrap">
                        <input type="password" name="password" id="inputPassGuru" placeholder="Buat password" required autocomplete="new-password">
...
<div id="modalResetPembeli"
    ...
    <div style="position:relative;background:#fff;border-radius:16px;padding:28px;width:90%;max-width:360px;box-shadow:0 8px 32px rgba(0,0,0,.15)">
        ...
        <form method="POST" id="formResetPembeli">
            ...
            <div class="form-group">
                ...
                <div class="password-wrap">
                    <input type="password" name="pw_reset" id="inputPwResetPembeli" placeholder="Masukkan password baru" required>
